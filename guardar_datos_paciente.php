...
function obtenerProximoIdPaciente($conn)
{
    ...
    return empty($ultimoIdPaciente) ? 1 : $ultimoIdPaciente + 1;
}
...
function guardarPacientesVacunas($conn, $pacientesVacunas, $proximoIdPaciente)
{
    foreach ($pacientesVacunas as $vacuna) {
        ...
        $sql_pacientes_vacunas = "INSERT INTO pacientes_vacunas (id_paciente, id_vacuna, dosis, refuerzo, FECHA_APLICACION) VALUES ($proximoIdPaciente, '$idVacuna', '$dosis', '$refuerzo', '$fechaAplicacion')";
...
$sql_paciente = "INSERT INTO paciente (id_paciente, nombre, apellido, sexo, fecha_nacimiento, Nacionalidad, Con_quien_vive, Direccion_reside) VALUES ($proximoIdPaciente, '$nombre', '$apellido', '$sexo', '$fecha_nacimiento', '$pais', '$con_quien_vive', '$direccion')";

if ($conn->query($sql_paciente) === TRUE) {
    echo "Datos del paciente guardados exitosamente.";
    
    // Verificar si se recibió el Id_seguro_salud
    if (!empty($Id_seguro_salud)) {
        ...
        if ($conn->query($sql_seguro_paciente) === TRUE) {
            echo "Datos del seguro del paciente guardados exitosamente.";
        } else {
            echo "Error al guardar los datos del seguro del paciente: " . $conn->error;
        }
    }
    ...
    echo "Error al guardar los datos del paciente: " . $conn->error;
}
