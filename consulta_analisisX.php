    ...
    <tbody>
      ...
      if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          echo "<tr onclick=\"seleccionarmedicamento('" . $row["id_analisis"] . "', '" . $row["Nombre"] . "', '" . $row["descripcion"] . "')\">";
  ...
  <script>
    $(document).ready(function() {
      ...
      $("#tabla_medicamento tbody").on("click", "tr", function() {
        ...
        var idanalisis = celdas.eq(0).text();
    ...
    window.parent.document.getElementById("id_analisis").value = idanalisis;
 window.parent.document.getElementById("nombre_analisis").textContent = nombre;
 window.parent.document.getElementById("descripcion_analisis").textContent = descripcion;
