...
<body>
  <fieldset>
    ...
    <h2 style="padding:0; text-align: center; text-transform: uppercase;">Consulta de Médicos X horarios</h2>
...
<table id="tabla_medicos" class="display">
  ...
  <tbody>
    <?php
    // Asumiendo que la consulta original para médicos ya ha sido ejecutada y guardada en $result
    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        // Subconsulta para contar los horarios vinculados a cada médico
        $id_medico = $row["id_medico"];
        $query_horarios = "SELECT COUNT(*) AS conteo 
                           FROM horario 
                           WHERE id_medico = $id_medico";
        $result_horarios = $conn->query($query_horarios);
        $conteo = $result_horarios->fetch_assoc()['conteo'];
        ...
        echo "<td>" . $conteo . "</td>"; // Mostrar el conteo de horarios
        echo "<td><a class='btn btn-primary' href='obtener_horarios_medico.php?id_medico=" . $row["id_medico"] . "'><i class='fa-regular fa-calendar-days'></i> &nbsp;Horario</a></td>";
        echo "</tr>";
      }
    } else {
